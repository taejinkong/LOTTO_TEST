...
# 1~1240회 전체가 들어 있다(2026-09-06에 1~605회를 합쳤다).
# 파일명은 606회부터만 담던 시절의 것이라 실제 범위와 맞지 않는다.
...
def validation_summary() -> dict:
    return {
        ...
        "trainingBase": "1~1242회 전체(2026-09-06부터). 이전에는 606회 이후만 학습했다.",
        "variants": [
            {"label": "페어+주기 v1", "averagePercentile": 43.8, "medianPercentile": 42.4},
            {"label": "전체 규칙 통합 v2", "averagePercentile": 45.1, "medianPercentile": 45.6},
            {
                "label": "순수 무작위 기대 기준",
...
FULL_BACKTEST_CSV = Path("backtest_full_2_1242.csv")
FULL_BACKTEST_SEGMENTS = [(2, 100), (101, 300), (301, 600), (601, 900), (901, None)]
TOTAL_COMBOS = 8_145_060


def full_backtest_summary() -> dict | None:
    """전 회차 워크포워드 결과를 회차별 CSV에서 매번 다시 집계한다.

    백분위를 손으로 옮겨 적으면 30회 검증처럼 낡은 값이 남으므로, 앱에 보이는
    수치는 모두 이 CSV에서 파생시킨다. 무작위 95% 범위는 균등분포(표준편차
    100/√12)의 표본 평균 기준이다.
    """
    if not FULL_BACKTEST_CSV.exists():
        return None
    by_model: dict[str, dict[int, tuple[int, float]]] = {}
    with FULL_BACKTEST_CSV.open(encoding="utf-8") as fp:
        for rec in csv.DictReader(fp):
            by_model.setdefault(rec["model"], {})[int(rec["round"])] = (
                int(rec["est_rank"]), float(rec["percentile"]),
            )

    def stats(rounds: list[int], data: dict[int, tuple[int, float]]) -> dict:
        ps = sorted(data[r][1] for r in rounds)
        n = len(ps)
        half = 1.96 * 100 / 12 ** 0.5 / n ** 0.5
        mid = (ps[(n - 1) // 2] + ps[n // 2]) / 2
        return {
            "count": n,
            "averagePercentile": round(mean(ps), 1),
            "medianPercentile": round(mid, 1),
            "random95": [round(50 - half, 1), round(50 + half, 1)],
            "top100kHits": sum(data[r][0] <= 100_000 for r in rounds),
            "top100kExpected": round(n * 100_000 / TOTAL_COMBOS, 1),
        }

    models = [("np_no_num", "페어+주기 v1"), ("integrated_v2", "전체 규칙 통합 v2")]
    first = min(min(d) for d in by_model.values())
    last = max(max(d) for d in by_model.values())
    segments = []
    for lo, hi in FULL_BACKTEST_SEGMENTS:
        hi = hi or last
        row = {"label": f"{lo}~{hi}회"}
        for key, _ in models:
            row[key] = stats([r for r in sorted(by_model[key]) if lo <= r <= hi], by_model[key])
        segments.append(row)

    v1, v2 = by_model["np_no_num"], by_model["integrated_v2"]
    common = sorted(set(v1) & set(v2))
    diffs = [v2[r][1] - v1[r][1] for r in common]
    sd = (sum((x - mean(diffs)) ** 2 for x in diffs) / (len(diffs) - 1)) ** 0.5
    return {
        "rounds": f"{first}~{last}회",
        "sampleSize": 20_000,
        "excluded": "1회는 이전 데이터가 없어 학습할 수 없으므로 제외",
        "models": [{"key": k, "label": label, **stats(sorted(by_model[k]), by_model[k])} for k, label in models],
        "segments": segments,
        "v2MinusV1": round(mean(diffs), 2),
        "v2MinusV1T": round(mean(diffs) / (sd / len(diffs) ** 0.5), 2),
        "v2BetterRounds": sum(x < 0 for x in diffs),
        "source": FULL_BACKTEST_CSV.name,
    }


def insights(validation: dict, full: dict | None = None) -> list[dict]:
    """검증 수치는 validation_summary() 하나만 보고 만든다.

    과거에 백분위를 이 함수에도 따로 적어 두는 바람에, 백테스트를 다시 돌린 뒤
    '모델 검증' 탭과 '인사이트' 탭이 서로 다른 숫자를 보여준 적이 있다.
    """
    by_label = {v["label"]: v for v in validation["variants"]}
    v2 = by_label["전체 규칙 통합 v2"]["averagePercentile"]
    v1 = by_label["페어+주기 v1"]["averagePercentile"]
    rounds = validation["rounds"]
    lo, hi = validation["randomMean95Range"]
    full_note = ""
    if full:
        f1, f2 = full["models"]
        full_note = (
            f" 검증 회차를 {full['rounds']} 전체({f1['count']:,}회)로 넓히면 v1 {f1['averagePercentile']}%, "
            f"v2 {f2['averagePercentile']}%로 무작위 기대치 50%(95% 범위 {f1['random95'][0]}~{f1['random95'][1]}%)와 "
            f"구별되지 않는다."
        )
    ...
    return [
        ...
        },
        {
            ...
            "tone": "warn",
            "body": (
                f"{rounds} 표본 검증에서 통합 v2 평균 백분위는 {v2}%, 페어+주기 v1은 {v1}%였고 "
                f"둘 다 Top 100,000 진입은 0회였다. 다만 이 정도 차이는 무작위여도 나온다. "
                f"30회 평균 백분위는 순수 무작위일 때도 약 95% 확률로 {lo}~{hi}% 안에 들어오며 "
                f"두 값 모두 그 범위 안이다.{full_note} 모든 단일 조합의 1등 확률은 1/8,145,060으로 같다."
            ),
        },
        {
            "title": "여러 장을 살수록 '겹침 상한'이 결과를 좌우한다",
            "tone": "good",
            "body": (
                "점수 상위만 뽑으면 같은 번호가 반복돼 여러 장이 동시에 죽는다. 예전 기본값인 "
                "겹침 최대 4개로 100장을 뽑으면 한 번호가 69장에 들어가고, 5등도 못 맞출 "
                "확률이 43.6%다. 겹침 상한을 2개로 조이면 14.5%, 번호별 사용 횟수 상한까지 "
                "걸면 7.4%로 내려간다. 200장이면 28.2% → 0.68% → 0.22%다(모두 전수 계산). "
                "기대 당첨 장수는 어느 쪽이든 장수 × 고정 확률로 같다. 바뀌는 것은 분산뿐이며, "
                "전멸 확률이 내려가는 대신 맞을 때 동시에 맞는 장수는 줄어든다."
            ),
        },
        {
            "title": "나눠 뽑지 말고 한 번에 뽑는다",
            "tone": "warn",
            "body": (
                "100장을 두 번 나눠 뽑으면 두 세트가 서로 겹치는 것을 막을 수 없다. 같은 "
                "200장인데도 5등조차 못 맞출 확률이 1.2~1.3%였고, 한 번에 200장으로 뽑으면 "
                "0.22~0.25%였다. 전수 계산 기준 약 6배 차이다. 그래서 한 번에 만들 수 있는 "
                "조합 수를 200개로 올렸다."
            ),
        },
        {
            "title": "번호 하나가 너무 많은 장에 들어가지 않게 한다",
            "tone": "good",
            "body": (
                "겹침 상한만 걸어도 특정 번호는 몰린다. 100장 기준으로 한 번호가 30장에 "
                "들어갔다. 번호별 사용 횟수 상한을 자동으로 조이면 그 값이 16장으로 내려가고, "
                "5등도 못 맞출 확률이 14.5%에서 7.4%로 절반이 된다. 상한으로 요청한 장수를 "
                "못 채울 때만 한 칸씩 푼다."
            ),
        },
        {
            "title": "고정 번호는 모든 장을 한 번호에 함께 건다",
            "tone": "warn",
            "body": (
                "고정 번호를 넣어도 표 한 장의 5등 이상 확률은 1/42로 같다. 달라지는 것은 "
                "장들 사이의 상관이다. 고정한 번호가 나오지 않는 회차(45개 중 6개만 나오므로 "
                "약 86.7%)에는 그 번호를 넣은 모든 장이 동시에 불리해진다. 겹침 상한 3개로 "
                "100장을 뽑을 때 5등도 못 맞출 확률은 고정 없음 12%, 한 개 고정 25%, "
                "두 개 고정 36%다. 고정 번호는 겹침 여유도 함께 잡아먹어, 겹침 상한 2개로는 "
                "한 개만 고정해도 100장을 채우지 못한다(57장이 한계)."
            ),
        },
        {
            "title": "전멸 확률은 추정이 아니라 전수 계산",
            "tone": "info",
            "body": (
                "생성 결과의 '5등도 못 맞출 확률'은 시뮬레이션이 아니다. 뽑은 조합들이 덮는 "
                "3개-조합을 하나도 포함하지 않는 회차를 8,145,060개 전체에서 정확히 세어 "
                "구한다. 200장이면 1밀리초, 가지치기가 가장 약한 6장에서도 100밀리초 아래다."
            ),
        },
        {
        ...
        {
            "title": "구간을 잘라 봐도 규칙은 나오지 않았다",
            "tone": "info",
            "body": (
                "'특정 구간에서는 어떤 패턴이 있지 않을까'를 다중비교까지 통제해 검정했다. "
                "1~1240회를 길이 60·120·250·400회의 모든 구간 4,134개로 훑어 각 특성이 "
                "전체 평균에서 가장 크게 벗어난 값을 구한 뒤, 회차 순서만 섞은 데이터로 "
                "같은 스캔을 5,000번 반복해 비교했다. 섞으면 시간 구조만 사라지므로 그 "
                "분포가 '규칙이 없을 때 스캔이 만들어내는 착시의 크기'가 된다. "
                "검정한 12개 특성(1~12 개수, 홀수 개수, 합계, 최대 연속 길이, 연속 쌍, "
                "끝수·번호대 종류, AC, 첫 번호, 평균 간격, 직전 회차 이월수) 가운데 "
                "11개가 무작위 범위였고, 유일하게 걸린 '최대 연속 길이'(p=0.049)도 "
                "12개 동시 검정 기준(p<0.0042)을 통과하지 못했다. "
                "특히 무작위로 섞은 데이터에서도 가장 극단적인 구간은 표준편차의 "
                "4.7~5.5배까지 벌어졌다. 구간을 훑다가 크게 튀는 구간을 발견해도 그것만으로는 "
                "규칙의 근거가 되지 않는다는 뜻이다. 도구는 segment_consistency.py 다."
            ),
        },
        {
...
def main() -> None:
    ...
    full_backtest = full_backtest_summary()
    payload = {
        "meta": {
        ...
        "insights": insights(validation, full_backtest),
    }
