...
def insights(validation: dict) -> list[dict]:
    ...
    return [
        ...
        },
        {
            ...
            "tone": "good",
            "body": (
                "점수 상위만 뽑으면 같은 번호가 반복돼 여러 장이 동시에 죽는다. 실제로 100장을 "
                "겹침 최대 4개로 뽑으면 서로 다른 3개-조합을 2,000개 중 1,166개만 덮고 5등도 "
                "못 맞출 확률이 약 44%다. 같은 후보 풀에서 겹침 상한만 2개로 낮추면 2,000개를 "
                "전부 덮고 그 확률이 약 14%로 내려간다. 200장이면 약 35%에서 0.7% 수준이 된다. "
        ...
        },
        {
            ...
            "body": (
                ...
                "약 86.7%)에는 그 번호를 넣은 모든 장이 동시에 불리해진다. 100장을 5번 고정으로 "
                "뽑으면 5등도 못 맞출 확률이 약 44%에서 52%로 오른다."
            ),
        },
        {
